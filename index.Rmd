...
---
# ¿Cómo instalar R y Rstudio en nuestros computadores?
<br><br>
...
La función ***select()***, que permite seleccionar de manera rápida variables de un **data.frame()** o **tibble()**, puede estar acompañada de funciones auxiliares que permiten seleccionar subconjuntos de variables a partir de patrónes o regularidades presentes en los nombres. A continuación, se presenta el listado de funciones auxiliares que pueden ser usadas en conjunto con la función ***select()***.
...
one_of() # Revisar el siguiente ejemplo df %>% select(-one_of('var1', 'var1'))
num_range() # Selección de variables por un patrón
last_col() # Seleccione la última variable 
everything() # Seleccionar todas las variables de un conjunto de datos
where() # Para la selección de variables según tipologías
```  
...
---
exclude: false
# .center[Filtrar individuos - funciones if_all e if_any]

Estas son funciones que cumplen la misma tarea de la función across, aplicables a la función filter. Aunque se puede hacer uso de across en combinación con este verbo, su uso está en desuso.

Ver los siguientes ejemplos.

iris %>% filter(if_any(contains("Sepal"), ~. < 2.3))

iris %>% filter(if_all(contains("Sepal"), ~.x > 3.8))

Contrastar los resultados del anterior código y 
ver el mensaje de advertencia que sale.

iris %>% filter(across(contains("Sepal"), ~. > 3.8))
...
El operador pipe de R, denotado por el simbolo **%>%**, nos permite unir varias consultas en una única cadena de código.  
  ...
  .panel[.panel-name[Representación Gráfica]
  
```{r , echo=FALSE, out.width="40%", fig.align='center'}
knitr::include_graphics("imagenes/Pipe.gif")
```
[Fuente: Arthur Welle](https://twitter.com/i/status/1535429654760284161)
...
---
exclude: true
#### .center[Pivotar a lo largo]

```{r , echo=FALSE, out.width="90%", fig.align='center'}
knitr::include_graphics("imagenes/pivot_longer.gif")
```
[Fuente: Vishal Katti](https://www.vishalkatti.com/posts/2022-08-29-tidyrpivotwider/)

---
exclude: true
#### .center[Pivotar a lo ancho]

```{r , echo=FALSE, out.width="100%", fig.align='center'}
knitr::include_graphics("imagenes/pivot_wider.gif")
```
[Fuente: Vishal Katti](https://www.vishalkatti.com/posts/2022-08-29-tidyrpivotwider/)
...
Revisar el siguiente sitio [web](https://www.codingprof.com/3-ways-to-replace-nas-with-zeros-in-r-examples/#:~:text=The%20classic%20way%20to%20replace,and%20assign%20them%20a%20zero.)
...
* Cuando los datos faltantes se encuentran en un archivo aparte/diferente, se recomienda usar la función [rows_update](https://twitter.com/cghlewis/status/1547917884776624130?s=27&t=wT2bOekocdkHRllauMcJ8Q) de dplyr.


---
exclude: true

# .center[Gestión/revisión de datos atípicos]

* Revisar, en principio, el siguiente artículo/sitio wed. [Ir al enlace](https://www.maximaformacion.es/blog-dat/como-lidiar-con-los-datos-atipicos-outliers/)
...
## .center[Manaul de uso de aes() en ggplot2]

[Manual aes()](https://ggplot2tor.com/)
...
## Paquetes para gestionar facetas en ggplot2
...
* [Enlace 3](https://eliocamp.github.io/ggnewscale/)



---
exclude: false
# Colores en ggplot2 - Slide 2

### Paquetes y otras posibilidades de colores en ggplot2

* [Enlace 4](https://stat545.com/colors.html)
* [Enlace 5](https://www.r-bloggers.com/2022/06/how-to-use-multiple-colour-scales-in-ggplot-with-ggnewscale/)
...
## .center[Características de un tema en ggplot2 - Opción 3]

```{r , echo=FALSE, out.width="65%", fig.align='center'}
knitr::include_graphics("imagenes/ThemeGgplot2V3.png") 
```

*Fuente de la imagen: *[cmdline](https://cmdlinetips.com/2021/05/tips-to-customize-text-color-font-size-in-ggplot2-with-element_text/)

---
exclude: true

## .center[Otros enlaces de interés para personalizar un tema en ggplot2]

* [Líneas](https://cmdlinetips.com/2021/05/tips-to-customize-lines-in-ggplot2-with-element_line/)
* [Rectangulos](https://cmdlinetips.com/2021/06/7-tips-to-customize-rectangle-elements-in-ggplot2-element_rect/)
* [Textos, fondos, tamaños](https://cmdlinetips.com/2021/05/tips-to-customize-text-color-font-size-in-ggplot2-with-element_text/)
* [Mejores diagraams de dispersión](https://cmdlinetips.com/2019/11/9-tips-to-make-better-scatter-plots-with-ggplot2-in-r/)
...
---
exclude: true
# Addings para el trabajo con ggplot2

1. El addin/paquete [esquisse](https://dreamrs.github.io/esquisse/index.html) permite crear gráficos en **ggplot2** de manera interactiva, así como recuperar sus respectivos códigos.

---
exclude: true
## Enlaces para el trabajo con etiquetas de longitud larga

Ir al siguiente [sitio web](https://www.andrewheiss.com/blog/2022/06/23/long-labels-ggplot/) en donde se presentan varios ejemplos sobre cómo tratar con etiquetas largas en el eje x.
...
---
exclude: true
## .center[Articulo paquetes análisis exploratorio en R]

En este [link](https://arxiv.org/pdf/1904.02101.pdf), se encuentra un artículo con listado y estudio de impacto de los principales paquetes existentes en R para el análisis exploratorio.
...
Una vez que hemos calculado los coeficientes de correlación de Pearson y Spearman, según sea el caso (presencia de normalidad o no normalidad), el paso final, para efectos de los procesos de investigación, consiste en evaluar si estos coeficientes son **estadísticamente significativos**. Es decir, **diferentes de cero** con cierto **nivel de certeza (significancia estadística)**. Para ello, debemos aplicar las respectivas pruebas de hipótesis.
  
#### Hipótesis Coefciente de Correlación de Pearson
...
Evaluar, con una *significancia del 5%*, si el coeficiente de correlación de Pearson $r$ entre el largo de la huella del pie izquierdo y la altura de los 1020 tamiles indios estudiados en el [ejemplo](#Pearson) de aplicación del coeficiente de Pearson, es estadísticamente diferente de cero.
]

  .panel[.panel-name[Prueba C. Pearson]
...
```{r, echo=TRUE, eval=TRUE, warning=FALSE}
# Código para evaluar la hipótesis

cor.test(x=Pies$foot, y=Pies$height, method="pearson")
```
]
...
Evaluar, con una *significancia del 5%*, si el coeficiente de correlación de Spearman $\rho$ entre los resultados obtenidos por los estudiantes en la prueba de comunicación escrita (PUNT_COMU_ESCR) y en la prueba de lectura crírica (PUNT_LECT_CRIT), y que se presentó en el [ejemplo](#Spearman) de aplicación del coeficiente de Spearman, es estadísticamente diferente de cero.

]
  .panel[.panel-name[Prueba C. Spearman]
...
cor.test(x=SaberPro$PUNT_COMU_ESCR, y=SaberPro$PUNT_LECT_CRIT, method="spearman")
```
...
1. Evaluar, haciendo uso del **Coeficiente de correlación correspondiente**, la correlación existente entre los resultados obtenidos en la prueba de **inglés** (PUNT_INGLES) y el puntaje obtenido en la prueba de lectura crítica (PUNT_LECT_CRIT) del conjunto de datos **SaberPro**. Una vez evaluada esta correlación y aplicando una **prueba de hipótesis pertinente**, evaluar si dicha correlación es estadísticamente diferente de cero (significancia del 5%). 
...
Esta pregunta, pocas veces se hace en el ámbito de la investigación y su respuesta es sumamente importante a la hora de evaluar *el grado y la intensidad* de la correlación lineal existente entre dos variables de tipo numérico.

En el siguente enlace, se puede acceder a un artículo de los autores Ortiz. J. y Ortiz. A. de la Revista de [Comunicaciones en Estadística](https://revistas.usantotomas.edu.co/index.php/estadistica/) de la Universidad Santo Tomas en Colombia, en donde se explora una posible respuesta a esta inquietud.

[Ver y descargar artículo](https://revistas.usantotomas.edu.co/index.php/estadistica/article/view/6769)

---
exclude: false
name: PruebaJi

### Asociación entre dos variables categóricas - Prueba independencia Chi Cuadrado

.panelset[

  .panel[.panel-name[Asociación]
 
 La asociación estadística entre dos variables categóricas hace referencia al grado de **relación existente** que existe entrelas **modalidades** (posibles respuestas) de las variables analizadas.
 
 Para evaluar la presencia o asuencia de relación entre dos variables de tipo categórico, existen pruebas llamadas de *independencia estadística* entre las cuales, una de las más conocidas, es la prueba de independencia **chi-cuadrado**.
  
  ]
  
  .panel[.panel-name[Var Categóricas]
  
Como se mencionó en la sección de [tipos de variables](#variables), las variables categóricas pueden clasificarse en:
...
$E:$ la frecuencia esperada.
  ...
  Aunque pocos, la prueba de independencia chi-cuadrado para dos variables de tipo categórico exije el cumplimiento de algunos supuestos entre los que se destacan.
  
  1. Al menos **el 80%** de las frecuencias esperadas debe ser **igual o superior a 5**.
  2. Las opciones de respuesta (modalidades o niveles) de las variables involucradas **no debe ser demasiado alto**.
  3. En muestras demasiado grandes, la prueba es altamente sensitiva en favor de la hipótesis alternativa o del investigador. En estos casos, se sugiere una constrastación/revisión gráfica.
  
  ]

  .panel[.panel-name[Ejemplo]
  
  [Ejemplo Prueba Chi Cuadrado de Independencia]()
  
Evaluar, con una *significancia del 5%* y haciendo uso de la prueba **Chi Cuadrado de Independencia**, si existe **asociación estadística** entre el tipo de colegio (COLEGIO) y el estrato socioeconómico (ESTRATO) del los estudiantes que hacen parte del conjunto de datos **SaberPro** bajo estudio.
    ]
    
  .panel[.panel-name[R. supuestos]
...
# Revisar función table() de la base del lenguaje
```

```{r janitor, echo=TRUE, eval=FALSE}
# Código Tabla Cruzada - Haciendo uso de librería Janitor
SaberPro %>%
  tabyl(COLEGIO, ESTRATO) %>%  
  adorn_percentages("all") %>%
  adorn_pct_formatting(digits = 2) %>%
  adorn_ns()
```

```{r ref.label="janitor", echo=FALSE} 
```
    ]
    
  .panel[.panel-name[Cod. Tabla]


```{r tablaji, echo=TRUE, eval=FALSE}

# Código Transformación de Datos - Para Ajuste de Supuestos.

Saber_Ji_Col <- SaberPro %>% 
                filter(COLEGIO %in% c("Oficial", "Privado"),
                                   ESTRATO != "ND/NE") %>% 
                mutate(ESTRATO_NEW = case_when(ESTRATO == "Estrato 1" ~ "Estrato 1",
                                               ESTRATO == "Estrato 2" ~ "Estrato 2",
                                               ESTRATO == "Estrato 3" ~ "Estrato 3",
                                                   TRUE ~ "Estrato 4 o más"))

#Código Tabla Cruzada - Cumplimiento Supuestos

Saber_Ji_Col %>%
  tabyl(COLEGIO, ESTRATO_NEW) |> 
...
### Tabla cruzada - Tipo de Colegio vs Estrato Socieconómico  
```{r ref.label= "tablaji", echo=FALSE, eval=TRUE}
```
    ]
    
    
  .panel[.panel-name[Cod. Gráfico]
...
#### Inspección Gráfica de Independencia/Dependencia

A continuación, se presneta el código de R haciendo uso del paquete **ggplot2** y la estética **geom_bar()** para barras apiladas.

```{r barchi, echo=TRUE, eval=FALSE}

ggplot(Saber_Ji_Col, aes(x = ESTRATO_NEW, fill = COLEGIO))+
  geom_bar(position = "fill") + 
  labs(title = "Diagrama de barras apiladas - Estrato vs Tipo de Colegio",
       x = " \nEstrato Socioeconómico",
       y = "Porcentaje")+
  geom_hline(yintercept=0.5, col = "blue", linetype="dashed", size = 1.3)+
...
```{r ref.label="barchi", echo=FALSE, warning=FALSE, fig.width = 8, fig.height = 5.5}
```
    ]
    
  .panel[.panel-name[Evaluación Prueba]

#### Código para la prueba Chi-cuadrado de Pearson

```{r, echo=TRUE, eval=FALSE}
# Código para la evaluación de una prueba chi-cuadrado en R

chisq.test(Saber_Ji_Col$ESTRATO_NEW, Saber_Ji_Col$COLEGIO)
```

#### Resultados prueba Chi-cuadrado de Pearson
...
```{r echo=FALSE}
countdown(minutes = 15, right = "0%", top = "10%", font_size = "2em")
```  
...
1. Evaluar, haciendo uso de la **prueba chi-cuadrado**, si existe **asociación estadística** entre el sexo (SEXO) y el puntaje obtenido en la prueba de razonamiento cuantitativo (PUNT_RAZO_CUANT) del conjunto de datos **SaberPro**. En el desarrollo del ejercicio, tener en cuenta los siguientes pasos.

* Crear una nueva variable llamada **RC** la cual cumple los siguientes criterios: **Bajo** si el puntaje obtenido en la prueba de razonamiento cuntitativo (PUNT_RAZO_CUANT) está entre 0 y 170 puntos y **Alto** si el puntaje obtenido en la prueba de razonamiento cuantitativo (PUNT_RAZO_CUANT) es mayor que 170 puntos.

* Teniendo en cuenta la nueva variable creada **RC**, evaluar la asociación de interés siguiendo los pasos tradicionales: revisión tabular, revisión gráfica, verificación y ajuste de supuestos y evaluación de la prueba de hipótesis de interés.
    ]
  ]
...
# `r emo::ji("oncoming")` <br> Modelamiento Estadístico <br> Regresión Lineal Simple y Múltiple

---
name: regresions
exclude: false

### Modelamiento Estadístico - Regresión Lineal Simple


.panelset[

  .panel[.panel-name[Regresión Simple]
  
La regresión lineal simple es una técnica de modelamiento estadístico para describir/predecir el comportamiento de una **variable numérica** de respuesta, también *llamada dependiente*, en función del comportamiento de una variable numérica independiente, también conocida como *variable predictora*. 

##### Filosofía de un modelo de Regresión Lineal Simple 

```{r , echo=FALSE, fig.align='center',  out.height="100%"}
knitr::include_graphics("imagenes/RegresionLineal.jpg") 
``` 
  
  ]
  
  .panel[.panel-name[El modelo]
  
El modelo de regresión lineal. 


  $$Y={\beta_{0}}+{\alpha_{1}X}+{\epsilon}$$
  
En donde, 

* $\beta_{0}$ es la intercepción de la recta con el eje $y$, esto es, la altura de la recta cuando $x=0$.
* $\beta_{1}$ es la pendiente de la recta que nos muestra cuando varia $y$ cuando nos movemos una unidad en el eje $x$ (de x = $x$ a x = $x+1$)
* $\epsilon_{i}$ son los errores aleatorios del modelo.
...
$\hat{\beta_{0}}=\bar{y}-\hat{\beta_{1}}\bar{x}$ &nbsp; y &nbsp; $\hat{\beta_{1}}=\frac{S_{xy}}{S_{xx}}$
  ]
  
  .panel[.panel-name[Supuestos]
 
La estimación/evaluación de un modelo de regresión lineal simple exige el cumplimiento de cuatro supuestos: **linealidad, homosedasticidad, normalidad e independencia**
 
 #### Supuestos Modelo de Regresión Lineal Simple
 
 * Debe existir una relación lineal entre la variable independiente (predictora) $x$ y la variable dependiente o de predicción $Y$ *(supuesto de linealidad)*.
 * Los residuos deben distribuirse de forma homogénea para todos los valores de la variable $Y$ *(supuesto de homosedasticidad)*.
 * Los residuales deben distribuirse de manera normal *(supuesto de normalidad)*.
 * los residuos deben distribuirse de manera independientes entre sí; es decir, no debe de existir algún tipo de correlación entre estos *(supuesto de independencia)*.
  ]
  
  .panel[.panel-name[Inferencia Estadística]
  
Para evaluar la calidad del ajuste (pertinencia) de un modelo de **regresión lineal simple**, en principio, debemos contrastar las siguientes hipótesis estadísticas sobre los coeficientes/parámetros del modelo propuesto:
  
$$\left\{\begin{array}{ll} H_{0}:& \beta_{i} = 0 \\ H_{1}:& \beta_{i} \neq 0, i=0,1\\ \end{array}\right.$$

* En donde se rechaza $H_{0}$, con una confianza de $1-\alpha$, cuando el $p-valor \leq \alpha$.
...
Explicar/predecir a través de un modelo de **Regresión Lineal Simple**, el comportamiento del salario **(Salario)** de una muestra de 15 individuos a partir de la nota **(Calificacion)** obtenida por estos en sus estudios universitarios. En el desarrollo del ejercicio, estimar los parámetros del modelo, evaluar las hipótesis asociadas a estos, así como verificar los supuestos del modelo de regresión ajustado.

[Descargar Muestra de Datos]()

Para el desarrollo de este ejercicio, en el boton siguiente y en formato Excel, pueden ser descargados los individuos que hacen parte de la muestra bajo estudio. Descargar este archivo e importarlo a nuestro entorno de trabajo **Rstudio**.

```{r echo=FALSE, eval=TRUE}

Regresion <- read_excel("Datos/Sesion2/Regresion.xlsx", 
    sheet = "Salarios")

Regresion %>%
  download_this(
    output_name = "RegresionS",
    output_extension = ".xlsx",
    button_label = "Ejp Regresión Lineal Simple - Descargar archivo Excel",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save")
```
  ]
  .panel[.panel-name[Rep. Gráfica]
  
```{r, echo=TRUE, xfig.width = 16, fig.height = 4.5}
ggplot(Regresion, aes(x = Calificacion, y = Salario)) +
         geom_point()+
  ggtitle("Diagrama de dispersión Salarios vs Calificación")
```
   ]
   
   .panel[.panel-name[Alt Modelo 1]
  
```{r, echo=TRUE, eval=FALSE}
# Ajuste a través de la función summary

summary(lm(Salario ~ Calificacion, data = Regresion))
```
...
```
  ...
  .panel[.panel-name[Alt Modelo 2]
  
```{r fit2, echo=TRUE, eval=TRUE}
# Para hacer uso de la función tidy, instalar el paquete broom

# install.packages("broom")
# library(broom)

# Ajuste del modelo
fit2 <- lm(Salario ~ Calificacion, data = Regresion)

# Resultados del modelo con tidy
tidy(fit2)
```
    ]
...
El modelo estimado o recta de regresión lineal de nuestro ejemplo, viene dado por:
<br>
$$\widehat{\text{Salario}} = -6.63 + 8.22*\text{Calificación}$$
<br>
Esto es, un punto de más en la calificación obtenida por un empleado en sus estudios universitarios, aumenta en 8.22 unidades su salario promedio. A continuación, se representa de manera gráfica el modelo ajustado.
]
  
  .panel[.panel-name[Gráfico Modelo]  
 
```{r, echo=TRUE, xfig.width = 16, fig.height = 2.5}
# Para construir este gráfico, se requiere instalar el paquete sjPlot
# install.packages("sjPlot")
# library(sjPlot)

# Gráfico del ajuste
plot_model(fit2,"pred", terms = ~Calificacion, 
                ci.lvl = NA, 
                show.data = TRUE, 
                axis.title = c("Calificación en los estudios universitarios", "Salario"),
                title = "Modelo Ajustado")
```
 
  ]
  .panel[.panel-name[Eval. Supuestos]
  
```{r, echo=TRUE, eval = FALSE, warning=FALSE}
# Ajuste del modelo
fit2 <- lm(Salario ~ Calificacion, data = Regresion)  

# Cálculo de los residuales (errores)
Residual <- tidy(residuals(fit2)) %>% rename(Error = x)

# Función de densidad de los residuales (errores)
den_res <- ggplot(Residual, aes(x = Error)) +
          geom_density()

# qqplot para los residuales (errores)
qq_res <- ggplot(Residual, aes(sample = Error)) +
  stat_qq() +
  stat_qq_line(col = "red")

# Mostrar gráficos en una única columna
grid.arrange(den_res, qq_res, ncol = 1)

# Implementación prueba de normalidad para residuales
shapiro.test(Residual$Error)
```
  ]
...
```{r echo=FALSE}
countdown(minutes = 30, right = "0%", top = "13%", font_size = "2em")
```  
...
1. Explicar/predecir/ajustar, haciendo uso de un **modelo de regresión lineal simple**, los resultados promedio globales (PUNTAJE_GLOBAL) obtenidos por los estudiantes del conjunto de datos SaberPro en función de los resultados obtenidos por estos en la prueba de razonamiento cuantitativo (PUNT_RAZO_CUANT). Ajustar el modelo, evaluar las hipótesis asociadas a este, graficar el modelo resultante, así como verificar  el cumplimiento de los supuestos exigidos por este tipo de modelo estadístico.
...
### Modelamiento Estadístico - Regresión Múltiple

.panelset[

  .panel[.panel-name[Regresión Múltiple]
  
La regresión lineal múltiple, al igual que la simple, es una técnica de modelamiento estadístico para describir/predecir/explicar el comportamiento de una **variable numérica** de respuesta $Y$, también *llamada variable dependiente*, en función del comportamiento de **dos o más variables** numéricas o categóricas, también conocidas como *variables independientes* o *predictoras*. 

##### Filosofía de un modelo de Regresión Lineal Múltiple 
...
``` 
 ...
 El modelo de regresión lineal múltiple está conformado por la siguiente ecuación. 
<br>
<br>
...
* $\beta_{0}$ es la intercepción con el eje $y$, esto es, la altura de la recta cuando todas la variables predictoras $x$ son iguales a $0$.
* $\beta_{i}$ son las pendientes de las rectas asociadas con cada variable predictora $x$ y representa el aumento o disminución del valor de la respuesta ($y$) cuando aumentamos en **una unidad** el valor de la variable predictora $x$ respectiva.
* $\epsilon$ son los errores aleatorios del modelo.
   ]
   
  .panel[.panel-name[Los supuestos]

La estimación/evaluación de un modelo de regresión lineal múltiple, al igual que en el caso de un modelo simple, exige el cumplimiento de cuatro supuestos: **linealidad, homosedasticidad, normalidad e independencia**
 
 #### Supuestos Modelo de Regresión Lineal Múltiple
 
 * Debe existir una relación lineal entre las variable independientes (predictoras) $x$ y la variable dependiente o de predicción $Y$ *(supuesto de linealidad)*.
 * Las variables independientes, como su nombre lo indica, deben ser independientes entre sí *(supuesto de no multicolinealidad)*.
 * Los residuos deben distribuirse de forma homogénea para todos los valores de la variable $Y$ *(supuesto de homosedasticidad)*.
 * Los residuales deben distribuirse de manera normal *(supuesto de normalidad)*.
 * los residuos deben distribuirse de manera independientes entre sí; es decir, no debe de existir algún tipo de correlación entre estos *(supuesto de independencia)*.
...
Para evaluar la calidad del ajuste (pertinencia) de un modelo de **regresión lineal múltiple**, en principio, debemos contrastar las siguientes hipótesis estadísticas sobre los coeficientes/parámetros del modelo propuesto:
  
$$\left\{\begin{array}{ll} H_{0}:& \beta_{i} = 0 \\ H_{1}:& \beta_{i} \neq 0, i=0,1\\ \end{array}\right.$$

* En donde se rechaza $H_{0}$, para cada $\beta_{i}$, con una confianza de $1-\alpha$, cuando los correspondientes $p-valores \leq \alpha$.
   ]
...
Explicar/predecir/evaluar, a través de un modelo de **Regresión Lineal Múltiple**, el comportamiento de una variable hipotética $Y$ de una muestra conformada por 32 individuos en función del comportamiento de cuatro variables predictoras: $x1, x2, x3$ y $x4$.
...
RegresionM <- read_excel("Datos/Sesion2/Regresion.xlsx", 
    sheet = "Gasolina")

RegresionM %>%
  download_this(
    output_name = "RegresionM",
    output_extension = ".xlsx",
    button_label = "Ejp Regresión Lineal Múltiple - Descargar archivo Excel",
    button_type = "warning",
    has_icon = TRUE,
    icon = "fa fa-save")
```
  ]
  .panel[.panel-name[Cod. Gráfica]
  
```{r grafRM, echo=TRUE, eval=FALSE}
# Para hacer uso de la función tidy, instalar el paquete tidyr

# install.packages("tidyr")
# library(tidyr)

# Transformación de datos y construcción de gráfico

RegresionM %>% pivot_longer(cols = c(x1:x4) , 
                            names_to = "Variables",
                            values_to = "Resultados") %>% 
  ggplot(aes(x = Resultados, y = Y))+
  geom_point()+
  geom_smooth(method = "lm", se = FALSE)+
  facet_wrap(~Variables, scales = "free_x")+
  ggtitle("Diagramas de dispersión entre variable Y vs variables x")
```
   ]
  .panel[.panel-name[Rep. Gráfica]
  
```{r ref.label="grafRM", echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.width = 10, fig.height = 6}

```
 
  ]
  
  .panel[.panel-name[Est. Modelo]
  
```{r fit3, echo=TRUE, eval=TRUE}
# Para hacer uso de la función tidy, instalar el paquete broom
...
El modelo estimado o recta de regresión lineal múltiple de este ejemplo, viene dado por:
<br>
<br>
$$\widehat{\text{Y}} = -6.8208 + 0.2272*\text{x1}+0.5537*\text{x2}-0.1495*\text{x3}+0.1547*\text{x4}$$
]
...
# Cálculo de los residuales (errores)
Residual_RM <- tidy(residuals(fit3)) %>% rename(Error = x)

# Función de densidad de los residuales (errores)
den_res_RM <- ggplot(Residual_RM, aes(x = Error)) +
          geom_density()

# qqplot para los residuales (errores)
qq_res_RM <- ggplot(Residual_RM, aes(sample = Error)) +
  stat_qq() +
  stat_qq_line(col = "red")

# Mostrar gráficos en una única columna
grid.arrange(den_res_RM, qq_res_RM, nrow = 1)

# Implementación prueba de normalidad para residuales
shapiro.test(Residual_RM$Error)
```
  ]
  .panel[.panel-name[Ejercicio]
  
```{r echo=FALSE}
countdown(minutes = 20, right = "0%", top = "13%", font_size = "2em")
```  
...
1. Explicar/predecir/ajustar, haciendo uso de un **modelo de regresión lineal múltiple**, los resultados promedio globales (PUNTAJE_GLOBAL) obtenidos por los estudiantes del conjunto de datos **SaberPro** en función de los resultados obtenidos por estos en las pruebas de razonamiento cuantitativo (PUNT_RAZO_CUANT), comunicación escrita (PUNT_COMU_ESCR), inglés (PUNT_INGLES) y lectura crítica (PUNT_LECT_CRIT). Ajustar el modelo, evaluar las hipótesis asociadas a este, así como verificar  el cumplimiento de los supuestos exigidos por este tipo de modelo estadístico. <br><br> **¿Qué podemos concluir acerca de los resultados promedio globales en el examen SaberPro con base en los resultados del modelo ajustado?**
  ]
  ]
...
# Para tener en cuenta en un nuevo curso - parte 1

1. [Presentación paso a paso (1 a 1) de las diferentes capas de un ggplot2](https://rstudio-conf-2020.github.io/design-ds-classroom/materials/01-curriculum-design/slides/01-curriculum-design.html#1). En la parte final de esta presentación se tiene un ejemplo.

Contrastar/ver/revisar el paquete [flipbookr](https://evamaerey.github.io/flipbookr/index.html) el cual cumple la misma función. Ver un ejemplo de este paquete en la parte final de esta presentación (ALTERNATIVA 2).
...
6. Revisar como adicionar porcentajes en barras apiladas.

7. Revisar el test de normalidad que se encuentra en el siguiente [enlace](https://www.learnbymarketing.com/tutorials/linear-regression-in-r/)

8. Revisar a detalle y para el tema de regresión, el paquete library(sjPlot)

---
exclude: false
# Para tener en cuenta en un nuevo curso - parte 2

9. Revisar el siguiente paquete de gráficos en ggplot2 con detalles estadísticos (ggstatsplot). https://indrajeetpatil.github.io/ggstatsplot/

10. Ver la posibilidad de insertar las respuestas de los ejercicios de manera web a través del paquete [webexercises](https://psyteachr.github.io/webexercises/). En la parte final del archivo de Trabajos RMD se encuentra disponible un ejemplo para el caso de un documento RMD.

11. Tener en cuenta, para el caso de la función filter, el uso de la función [between()](https://dplyr.tidyverse.org/reference/between.html) de **dplyr** para la selección de un conjunto de individuos que se encuentran dentro de un rango de observaciones de una variable numérica.
Ejp.
filter(starwars, between(height, 100, 150))

12. Incluir/analizar la inclusión de la función **[recode()](https://dplyr.tidyverse.org/reference/recode.html)** de dplyr para la recodificación en la misma o en una nueva variable dentro de la función mutate() como alternativa a la función case_when().

13. Revisar el siguiente sitio web en donde se presenta un resumen para las [pruebas de hipótesis de comparación](https://www.maximaformacion.es/blog-dat/guia-para-encontrar-tu-prueba-estadistica/). 

14. Incluir, en el componente del operador **PIPE** (%>%), el adding/paquete [ViewPipeSteps](https://github.com/daranzolin/ViewPipeSteps) el cual permite mostrar todos los conjuntos de datos involucrados/generados al usar este operador.

---
exclude: false
# Para tener en cuenta en un nuevo curso - parte 3

15. Revisar, para el caso de los procesos de importación a R a través de un data.frame , el Paquete/adding [editData](https://cran.r-project.org/web/packages/editData/vignettes/editData.html) para editar un data.frame desde Rstudio.
Ver el siguiente ejemplo a través de un [GIT](https://twitter.com/RosanaFerrero/status/1541392148074860544)

16. Revisar/explorar el paquete [correlation](https://easystats.github.io/correlation/) para el caso de los análisis de correlación. Ver también el contenido del siguiente enlace. https://easystats.github.io/see/articles/correlation.html



<!-- Ejemplo PASO A PASO de un GGPLOT2-->
...
<!-- Ejemplo PASO A PASO de un GGPLOT2 - ALTERNATIVA 2-->

---
exclude: FALSE

`r chunk_reveal("my_iris")`
...
```{r my_iris, include = FALSE, message=FALSE, warning=FALSE, error=FALSE}
iris %>%
  select(Petal.Length, Sepal.Length, Species) %>% 
  filter(Species %in% c("virginica", "setosa")) %>%
  ggplot() +
  aes(x = Petal.Length) + #BREAK
  aes(y = Sepal.Length) + #BREAK
  geom_point(
    alpha = .8, size = 5) + 
  aes(color = Species)
```
